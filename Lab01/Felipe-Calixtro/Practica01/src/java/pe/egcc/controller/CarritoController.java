...
@WebServlet(name = "CarritoController",
        urlPatterns = {"/CarritoAgregar", "/CarritoListar","/CarritoEstadistica", 
...
public class CarritoController extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    }else if(path.equals("/CarritoEstadistica")){
        carritoEstadistica(request, response);
  ...
  private void carritoAgregar(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if(precio<0 ){
        ...
        // Proceso
    ItemDto dto = new ItemDto(cliente,producto, precio, cant, 0);
  ...
  private void carritoListar(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
   ...
   private void carritoEstadistica(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    // Acceso al carrito
    HttpSession session = request.getSession();
    if (session.getAttribute("carrito") == null) {
       System.err.println("EGCC: Carrito desde estadistica");
      session.setAttribute("carrito", new CarritoModel());
    }
    CarritoModel carrito;
    carrito = (CarritoModel) session.getAttribute("carrito");
    ...
    request.setAttribute("dtoMayor", carrito.getVentaMayorImporte());
    request.setAttribute("dtoMenor", carrito.getVentaMenorImporte());
    request.setAttribute("dtoMayorV", carrito.getMayorVenta());
    request.setAttribute("dtoMenorV", carrito.getMenorVenta());
    RequestDispatcher rd;
